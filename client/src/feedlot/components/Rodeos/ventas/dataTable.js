...
function createData(caravana,establecimiento,rodeo,raza,peso) {
  return {caravana,establecimiento,rodeo,raza,peso};
}
...
const headCells = [
  { id: 'caravana', numeric: false, disablePadding: false, label: 'Caravana' },
  { id: 'establecimiento', numeric: true, disablePadding: false, label: 'Establecimiento' },
  { id: 'rodeo', numeric: true, disablePadding: false, label: 'Rodeo' },
  { id: 'raza', numeric: true, disablePadding: false, label: 'Raza' },
...
const EnhancedTableToolbar = (props) => {
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        <Typography className={classes.title} color="green" variant="subtitle1" component="div">
...
//--------------SORT & SELECT VARIABLES---------------------------  
  const [order, setOrder] = React.useState('asc');
  const [orderBy, setOrderBy] = React.useState('caravana');
  const [selected, setSelected] = React.useState([]);
  const [page, setPage] = React.useState(0);
  const [dense, setDense] = React.useState(false);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
//------DATA VARIABLES HANDLERS AND FUNCTIONS----------------------------
          ...
          >
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.caravana);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.caravana)}
                      role="checkbox"
                    ...
                    >
                      ...
                      <TableCell align="right">{row.rodeo}</TableCell>
